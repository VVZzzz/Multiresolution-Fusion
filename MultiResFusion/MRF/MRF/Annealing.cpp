...
CAnnealing::CAnnealing(IplImage* img, int width, int height,int template_size)
{
	...
	cin>>m_final_imgsize;
	if (m_final_imgsize<height)
	{
		cout<<"重建尺寸小于训练图像！"<<endl;
		return;
	}
	m_multiple=m_final_imgsize/height;     //计算待重建尺寸与训练图像之间的倍数关系
		...
		for (i=1;i!=temporay_value_2-1;i++)      //3*3模板的中心点，判断分级图像是否存在3*3区域为同一相，只要存在，则继续寻找
		{
			for (j=1;j!=temporay_value_2-1;j++)
			{
				...
				for (p=-1;p!=2;p++)
				{
					for (q=-1;q!=2;q++)
					{
						sum=sum+m_initial_imgvec[(i+p)*temporay_value_1][(j+q)*temporay_value_1];
					}
				}
				if (sum==9 /*|| sum==0*/)
				{
...
void CAnnealing::Set_inital_data()
{
	...
	m_initial_rand_data.resize(new_inital_size);      //inital_rand_data初始最小容器随机赋值，
	for (i=0;i!=new_inital_size;++i)
	{
		...
		for (j=0;j!=new_inital_size;++j)
		{
			...
			{
				if (m_big_3Dimgvec[i*size_number][j*size_number][k*size_number]!=0)
				{
	...
	for (i=0;i!=m_Rec_min_grid_size;i++)
	{
		...
		{
			if (m_initial_imgvec[i*size_number][j*size_number]==1)    //Inital_image_vector存放原图容器（高分辨）
			{
				inital_white_point++;
			}
		}
	...
	for (i=0;i!=new_inital_size*inital_white_point*(m_multiple*m_multiple);i++)   
	{
		rand_site= rand32()%counter_number;
		if (m_initial_rand_data[point_x[rand_site]][point_y[rand_site]][point_z[rand_site]]!=300)   //190212
		{
			m_initial_rand_data[point_x[rand_site]][point_y[rand_site]][point_z[rand_site]]=1;//1表示白色点，0表示黑色点
	...
	for (i=0;i!=new_inital_size;i++)
	{
		for (j=0;j!=new_inital_size;j++)
		{
			for (k=0;k!=new_inital_size;k++)
			{
				if (m_initial_rand_data[i][j][k]!=1/*&&inital_rand_data[i][j][k]!=300*/)   //190212
				{
...
{
	m_train_code_value.clear();   //TI_code_value存放训练图像的多点密度函数             //将上一网格存储的多点密度函数清空
	int i=0,j=0;
		...
		for (j=0;j!=m_vector_size;++j)
		{
			...
			for (p=-k;p!=k+1;p++)
			{
				for (q=-k;q!=k+1;q++)
				{
					...
					code_value=code_value*2+Temporary_vector[(i+p+m_vector_size)%m_vector_size][(j+q+m_vector_size)%m_vector_size];
				}
			}
			pair<map<int,int>::iterator,bool> pair_code_value=m_train_code_value.insert(make_pair(code_value,1));
...
void CAnnealing::Set_inital_to_reconstruct_uncondition_2()
{
	...
	m_preproc_vector.resize(m_new_vector_size);     //存放预处理结果容器，标注3*3全为黑或者全白的区域 
	for (i=0;i!=m_new_vector_size;++i)
	{
		m_reconstruct_vec[i].resize(m_new_vector_size);
		...
		for (j=0;j!=m_new_vector_size;j++)
		{
			...
			for (k=0;k!=m_new_vector_size;k++)
			{
				if (m_big_3Dimgvec[i*distance_point][j*distance_point][k*distance_point]!=0)
				{
	...
	for (i=0;i!=m_vector_size;++i)
	{
		for (j=0;j!=m_vector_size;++j)
		{                             
			if (m_initial_imgvec[i*distance_point][j*distance_point]==1)              /////计算本层原图白色点数
			{
	...
	for (i=0;i!=m_new_vector_size/2;++i)   //190113
	{
		for (j=0;j!=m_new_vector_size/2;++j)
		{
			for (k=0;k!=m_new_vector_size/2;++k)
			{
				...
				{
					m_reconstruct_vec[2*i][2*j][2*k]=m_initial_rand_data[i][j][k];              /////将上层重建的结果传递到对应网格点
					m_preproc_vector[2*i][2*j][2*k]=m_initial_rand_data[i][j][k];              /////将上层重建的结果传递到对应网格点
	...
	/////////////////////////////////////////////////////
	for (i=0;i!=m_new_vector_size;++i)    //190113（处理奇数x、y、z对应的像素点！）
	{
		for (j=0;j!=m_new_vector_size;++j)
		{
			for (k=0;k!=m_new_vector_size;++k)
					...
					for (q=-1;q!=2;q++)    //i,j,k为奇数（奇+奇=偶、奇+偶=奇）
					{
						for (p=-1;p!=2;p++)
						{
							for (n=-1;n!=2;n++)    
							{
								...
								if (((i+q+m_new_vector_size)%m_new_vector_size)%2==0 && ((j+p+m_new_vector_size)%m_new_vector_size)%2==0 && ((k+n+m_new_vector_size)%m_new_vector_size)%2==0)
								{
									//q、p、n需要为奇数
									//满足条件的有(-1/1,-1/1,-1/1)共8个点，它们刚好为前一级映射过来的点
									sum=sum+m_reconstruct_vec[(i+q+m_new_vector_size)%m_new_vector_size][(j+p+m_new_vector_size)%m_new_vector_size][(k+n+m_new_vector_size)%m_new_vector_size];
								}
							}
						}
					}
					...
					if (sum==8)                          /////全1时，除该区域中心点外，其余点全赋值为1
					{
						for (q=-1;q!=2;q++)               //i,j,k为奇数（奇+奇=偶、奇+偶=奇）
						{
							for (p=-1;p!=2;p++)
							{
								...
								{
									//有一个公式满足和为奇数即可
									if (((i+q+m_new_vector_size)%m_new_vector_size)%2==1 || ((j+p+m_new_vector_size)%m_new_vector_size)%2==1 || ((k+n+m_new_vector_size)%m_new_vector_size)%2==1)
									{
										//q、p、n至少有一个为偶数，即0！
										//即满足由前一级映射过来的点全为1时，3*3*3模板中所有值为2的点（扩展点）
										if (m_preproc_vector[(i+q+m_new_vector_size)%m_new_vector_size][(j+p+m_new_vector_size)%m_new_vector_size][(k+n+m_new_vector_size)%m_new_vector_size]!=300)   //190212
										{
											m_preproc_vector[(i+q+m_new_vector_size)%m_new_vector_size][(j+p+m_new_vector_size)%m_new_vector_size][(k+n+m_new_vector_size)%m_new_vector_size]=31;
										}
									}
								}
							}
							...
							for (p=-1;p!=2;p++)
							{
								for (n=-1;n!=2;n++)
								{
									if (((i+q+m_new_vector_size)%m_new_vector_size)%2==1 || ((j+p+m_new_vector_size)%m_new_vector_size)%2==1 || ((k+n+m_new_vector_size)%m_new_vector_size)%2==1)
									{
										...
										//{
											m_preproc_vector[(i+q+m_new_vector_size)%m_new_vector_size][(j+p+m_new_vector_size)%m_new_vector_size][(k+n+m_new_vector_size)%m_new_vector_size]=30;     
										//}
									}
								}
							}
	...
	all_add_white_number=m_new_vector_size*current_grid_white_number*(m_multiple*m_multiple)-up_grid_white_number;         /////本级网格需要添加白色点个数
	all_add_black_number=m_new_vector_size*(m_new_vector_size*m_new_vector_size-current_grid_white_number*(m_multiple*m_multiple));
	...
	{
		for (i=0;i!=m_new_vector_size;++i)//先将3*3*3区域判断的点直接将这些满足条件的数据赋值
		{
			for (j=0;j!=m_new_vector_size;++j)
			{
				for (k=0;k!=m_new_vector_size;++k)
		...
		for (i=0;i!=m_new_vector_size;++i)//判断全1或者全0的2*2*2区域点个数
		{
			for (j=0;j!=m_new_vector_size;++j)
			{
				for (k=0;k!=m_new_vector_size;++k)
				{
					...
					if (i%2==0 && j%2==1 && k%2==1 && (m_preproc_vector[i][j][k]==2||m_preproc_vector[i][j][k]==300) )
					{
						sum=0;
						for (q=-1;q!=2;q++)     
						{
							...
							{
								//只考虑j、k，需要q、p为奇数
								if (((j+q+m_new_vector_size)%m_new_vector_size)%2==0 && ((k+p+m_new_vector_size)%m_new_vector_size)%2==0)
								{
									sum=sum+m_reconstruct_vec[i][(j+q+m_new_vector_size)%m_new_vector_size][(k+p+m_new_vector_size)%m_new_vector_size];
								}
							}
						}
						if (sum==4)
						{
							if (m_preproc_vector[i][j][k]!=300)    //190213：是否有点问题？Pre大孔所在的位置可能在上一部分已经赋值为30？！
					...
					else if (i%2==1 && j%2==1 && k%2==0 && (m_preproc_vector[i][j][k]==2||m_preproc_vector[i][j][k]==300) )
					{
						sum=0;
						for (q=-1;q!=2;q++)
						{
							for (p=-1;p!=2;p++)
							{
								if (((i+q+m_new_vector_size)%m_new_vector_size)%2==0 && ((j+p+m_new_vector_size)%m_new_vector_size)%2==0)
								{
									sum=sum+m_reconstruct_vec[(i+q+m_new_vector_size)%m_new_vector_size][(j+p+m_new_vector_size)%m_new_vector_size][k];
								}
							}
						}
						if (sum==4)
					...
					else if (i%2==1 && j%2==0 && k%2==1 && (m_preproc_vector[i][j][k]==2||m_preproc_vector[i][j][k]==300) )
					{
						sum=0;
						for (q=-1;q!=2;q++)
						{
							for (p=-1;p!=2;p++)
							{
								if (((i+q+m_new_vector_size)%m_new_vector_size)%2==0 && ((k+p+m_new_vector_size)%m_new_vector_size)%2==0)
								{
									sum=sum+m_reconstruct_vec[(i+q+m_new_vector_size)%m_new_vector_size][j][(k+p+m_new_vector_size)%m_new_vector_size];
								}
							}
						}
						if (sum==4)
		...
		if (all_add_white_number>=(white_point_number_3D+white_point_number_2D) && all_add_black_number>=(black_point_number_3D+black_point_number_2D))
		{
			...
			while (add_white_number_2D!=0)
			{
				...
				if ((point_y%2==1 || point_x%2==1 || point_z%2==1) && m_preproc_vector[point_x][point_y][point_z]==2 && m_reconstruct_vec[point_x][point_y][point_z]==2)
				{
		...
		else /////第二次2*2*2中心点直接赋值不满足孔隙度时，回退到上一步3*3*3设置状态
		{
			...
			while (add_white_number_3D!=0)
			{
				...
				point_y=rand32()%m_new_vector_size;
				point_z=rand32()%m_new_vector_size;
				if ((point_y%2==1 || point_x%2==1 || point_z%2==1) && m_preproc_vector[point_x][point_y][point_z]==2)
				{
					m_reconstruct_vec[point_x][point_y][point_z]=1;
					add_white_number_3D--;
				}
	...
	else   /////直接按照需要随机赋值
	{
		...
		while (all_add_white_number!=0)
		{
			...
			point_y=rand32()%m_new_vector_size;
			point_z=rand32()%m_new_vector_size;
			if ((point_y%2==1 || point_x%2==1 || point_z%2==1)  && m_preproc_vector[point_x][point_y][point_z]!=300)   //190212
			{
				m_reconstruct_vec[point_x][point_y][point_z]=1;
		...
		for (i=0;i!=m_new_vector_size;i++)
		{
			for(j=0;j!=m_new_vector_size;j++)
			{
				for (k=0;k!=m_new_vector_size;k++)
				{
					...
					m_preproc_vector[i][j][k]=m_reconstruct_vec[i][j][k];/////最后将Pre_process_vector作为指示容器
				}
			}
		}
	}
	...
	for (i=0;i!=m_new_vector_size;i++)
	{
		for(j=0;j!=m_new_vector_size;j++)
		{
			for (k=0;k!=m_new_vector_size;k++)
			{
				if (m_preproc_vector[i][j][k]!=2 && m_preproc_vector[i][j][k]!=300 && m_reconstruct_vec[i][j][k]==1)
				{                                     //190213：该条件好像不需要，因为reconst为1的位置一定不是大孔孔点位置
					m_preproc_vector[i][j][k]=1;
				}
				else if (m_preproc_vector[i][j][k]!=2 && m_reconstruct_vec[i][j][k]==0)
				{
		...
		for (j=0;j!=m_new_vector_size;++j)
		{
			for (i=0;i!=m_new_vector_size;++i)
			{
				...
				for (p=-s;p!=s+1;p++)
				{
					for (q=-s;q!=s+1;q++)
					{
						code_value=code_value*2+m_reconstruct_vec[k][(i+p+m_new_vector_size)%m_new_vector_size][(j+q+m_new_vector_size)%m_new_vector_size];
					}
				}
				pair_code_value=m_reconstruct_code_value.insert(make_pair(code_value,1));
				if(!pair_code_value.second)           
				{
					++pair_code_value.first->second; 
				}
				...
				for (p=-s;p!=s+1;p++)
				{
					for (q=-s;q!=s+1;q++)
					{
						code_value=code_value*2+m_reconstruct_vec[(i+p+m_new_vector_size)%m_new_vector_size][k][(j+q+m_new_vector_size)%m_new_vector_size];
					}
				}
				pair_code_value=m_reconstruct_code_value.insert(make_pair(code_value,1));
				if(!pair_code_value.second)           
				...
				for (p=-s;p!=s+1;p++)
				{
					for (q=-s;q!=s+1;q++)
					{
						code_value=code_value*2+m_reconstruct_vec[(i+p+m_new_vector_size)%m_new_vector_size][(j+q+m_new_vector_size)%m_new_vector_size][k];
					}
				}
				pair_code_value=m_reconstruct_code_value.insert(make_pair(code_value,1));
				if(!pair_code_value.second)           
...
void CAnnealing::Calculate_E_differ(int& E_differ)
{
	...
	for (i=0;i!=m_sizenum;i++) //size_number模板对应大小编码最大值
	{
		...
		E_differ=E_differ+abs(3*m_new_vector_size*m_train_code_value[i]*(m_multiple*m_multiple)-m_reconstruct_code_value[i]);
	}
	...
	for (i=0;i!=m_new_vector_size;i++)
	{
		for (j=0;j!=m_new_vector_size;j++)
		{
			for (k=0;k!=m_new_vector_size;k++)
			{
				...
				else if (m_big_3Dimgvec[i*distance_point][j*distance_point][k*distance_point]==0 && m_reconstruct_vec[i][j][k]==0)
	...
	for (i=0;i!=m_new_vector_size;i++)
	{
		for (j=0;j!=m_new_vector_size;j++)
		{
			for (k=0;k!=m_new_vector_size;k++)
			{
				...
				///////////////////////////////////////////////////////////////////
				if (i%2==1 && j%2==1 && k%2==1 && (m_preproc_vector[i][j][k]==2/*||Pre_process_vector[i][j][k]==300*/) ) //存放预处理结果容器，标注3*3全为黑或全白的区域
				{                                                                        //190212
					sum_xy=0;
					sum_xz=0;
					sum_yz=0;
					for (q=-1;q!=2;q++)
					{
						for (p=-1;p!=2;p++)
						{
							sum_xy=sum_xy+m_reconstruct_vec[(i+q+m_new_vector_size)%m_new_vector_size][(j+p+m_new_vector_size)%m_new_vector_size][k];
							sum_xz=sum_xz+m_reconstruct_vec[(i+q+m_new_vector_size)%m_new_vector_size][j][(k+p+m_new_vector_size)%m_new_vector_size];
							sum_yz=sum_yz+m_reconstruct_vec[i][(j+p+m_new_vector_size)%m_new_vector_size][(k+p+m_new_vector_size)%m_new_vector_size];
						}
					}
					if (m_reconstruct_vec[i][j][k]==1 && sum_xy!=9 && sum_xz!=9 && sum_yz!=9)
					{
						temporay_vector[i][j][k]=1;
					}
					else if (m_reconstruct_vec[i][j][k]==0 && sum_xy!=0 && sum_xz!=0 && sum_yz!=0)
					{
				...
				{                                                                            //190212
					...
					for (q=-1;q!=2;q++)
					{
						for (p=-1;p!=2;p++)
						{
							sum_xy=sum_xy+m_reconstruct_vec[(i+q+m_new_vector_size)%m_new_vector_size][(j+p+m_new_vector_size)%m_new_vector_size][k];
							sum_xz=sum_xz+m_reconstruct_vec[(i+q+m_new_vector_size)%m_new_vector_size][j][(k+p+m_new_vector_size)%m_new_vector_size];
							sum_yz=sum_yz+m_reconstruct_vec[i][(j+q+m_new_vector_size)%m_new_vector_size][(k+p+m_new_vector_size)%m_new_vector_size];
							for (g=-1;g!=2;g++)
							{
								sum_3D=sum_3D+m_reconstruct_vec[(i+q+m_new_vector_size)%m_new_vector_size][(j+p+m_new_vector_size)%m_new_vector_size][(k+g+m_new_vector_size)%m_new_vector_size];
							}
						}
					}
					if (sum_3D==27)
					...
					{
						if (sum_xy!=9 && (sum_xz!=9 || sum_yz!=9) && m_reconstruct_vec[i][j][k]==1)
						{
							if (m_preproc_vector[i][j][k]!=300)    //190212
							{
								temporay_vector[i][j][k]=1;
						...
						else if (sum_xy!=0 && (sum_xz!=0 || sum_yz!=0) && m_reconstruct_vec[i][j][k]==0)
						{
							if (m_preproc_vector[i][j][k]!=300)   //190212：大孔孔点位置不允许交换（只能为小孔的背景点）   //光看这一句会觉得：有些300变成20/30了呀？但大前提if限定了pre为2/300
							{
								temporay_vector[i][j][k]=0;
							}
						}
				...
				{                                                                            //190212
					...
					{
						if (sum_xz!=9 && (sum_xy!=9 || sum_yz!=9) && m_reconstruct_vec[i][j][k]==1)
						{
							if (m_preproc_vector[i][j][k]!=300)   //190212
							{
								temporay_vector[i][j][k]=1;
						...
						else if (sum_xz!=0 && (sum_xy!=0 || sum_yz!=0) && m_reconstruct_vec[i][j][k]==0)
						{
							if (m_preproc_vector[i][j][k]!=300)   //190212：大孔孔点位置不允许交换（只能为小孔的背景点）
							{
								temporay_vector[i][j][k]=0;
							}
						}
				...
				/////////////////////////////////////////////////////////////////////
				else if (i%2==0 && j%2==1 && k%2==1 && (m_preproc_vector[i][j][k]==2||m_preproc_vector[i][j][k]==300) )
				{                                                                            //190212
					sum_3D=0;
					sum_xy=0;
					sum_xz=0;
					...
					{
						if (sum_yz!=9 && (sum_xy!=9 || sum_xz!=9) && m_reconstruct_vec[i][j][k]==1)
						{
							if (m_preproc_vector[i][j][k]!=300)   //190212
							{
								temporay_vector[i][j][k]=1;
						...
						else if (sum_yz!=0 && (sum_xy!=0 || sum_xz!=0) && m_reconstruct_vec[i][j][k]==0)
						{
							if (m_preproc_vector[i][j][k]!=300)   //190212：大孔孔点位置不允许交换（只能为小孔的背景点）
							{
								temporay_vector[i][j][k]=0;
				...
				/////////////////////////////////////////////////////////////////////
				else if (i%2==0 && j%2==0 && k%2==1 && (m_preproc_vector[i][j][k]==2||m_preproc_vector[i][j][k]==300) )
				{                                                                            //190212
					sum_3D=0;
					sum_xy=0;
					sum_xz=0;
				...
				/////////////////////////////////////////////////////////////////////
				else if (i%2==0 && j%2==1 && k%2==0 && (m_preproc_vector[i][j][k]==2||m_preproc_vector[i][j][k]==300) )
				{                                                                            //190212
					sum_3D=0;
					sum_xy=0;
					sum_xz=0;
					...
					{
						if (m_reconstruct_vec[i][j][k]==1 && (sum_xz!=9 || (sum_xy!=9 && sum_yz!=9)))
						{
							if (m_preproc_vector[i][j][k]!=300)    //190212
							{
								temporay_vector[i][j][k]=1;
						...
						else if(m_reconstruct_vec[i][j][k]==0 && (sum_xz!=0 || (sum_xy!=0 && sum_yz!=0)))
						{
							if (m_preproc_vector[i][j][k]!=300)   //190212：大孔孔点位置不允许交换（只能为小孔的背景点）
							{
								temporay_vector[i][j][k]=0;
				...
				/////////////////////////////////////////////////////////////////////
				else if (i%2==1 && j%2==0 && k%2==0 && (m_preproc_vector[i][j][k]==2||m_preproc_vector[i][j][k]==300) )
				{                                                                            //190212
					sum_3D=0;
					sum_xy=0;
					sum_xz=0;
					...
					{
						if (m_reconstruct_vec[i][j][k]==1 && (sum_yz!=9 || (sum_xy!=9 && sum_xz!=9)))
						{
							if (m_preproc_vector[i][j][k]!=300)   //190212
							{
								temporay_vector[i][j][k]=1;
						...
						else if (m_reconstruct_vec[i][j][k]==0 && (sum_yz!=0 || (sum_xy!=0 && sum_xz!=0)))
						{
							if (m_preproc_vector[i][j][k]!=300)   //190212：大孔孔点位置不允许交换（只能为小孔的背景点）
							{
								temporay_vector[i][j][k]=0;
...
{ 
	...
	for (q=-k;q!=k+1;q++)      ////次循环为每个交换点决定的9个模式
	{
		for (p=-k;p!=k+1;p++)
		{
			m_reconstruct_vec[white_x][white_y][white_z]=1;//交换前(190212：坐标是由temporay_vector存放选择的交换点确定的)
			...
			for (i=-k;i!=k+1;i++) ////此循环为计算每个模式
			{
				for (j=-k;j!=k+1;j++)
				{
					code_value=code_value*2+m_reconstruct_vec[(i+white_x+q+m_new_vector_size)%m_new_vector_size][(j+white_y+p+m_new_vector_size)%m_new_vector_size][white_z];
				}
			}
			--m_exchange_code_value_white[code_value];//减去交换前由白色交换点确定的模式
			...
			if (white_z!=black_z)
			{
				...
				for (i=-k;i!=k+1;i++)
				{
					for (j=-k;j!=k+1;j++)
					{
						code_value=code_value*2+m_reconstruct_vec[(i+black_x+q+m_new_vector_size)%m_new_vector_size][(j+black_y+p+m_new_vector_size)%m_new_vector_size][black_z];
					}
				}
				--m_exchange_code_value_black[code_value];//减去交换前由黑色交换点确定的模式
			}

			else if (abs((black_x+q+m_new_vector_size)%m_new_vector_size-white_x)>k || abs((black_y+p+m_new_vector_size)%m_new_vector_size-white_y)>k)
			{
				code_value=0;
				for (i=-k;i!=k+1;i++)
				{
			...
			for (i=-k;i!=k+1;i++)
			{
				for (j=-k;j!=k+1;j++)
				{
					code_value=code_value*2+m_reconstruct_vec[(i+white_x+q+m_new_vector_size)%m_new_vector_size][white_y][(j+white_z+p+m_new_vector_size)%m_new_vector_size];
				}
			}
			--m_exchange_code_value_white[code_value];//减去交换前由白色交换点确定的模式
			...
			if (white_y!=black_y)
			{
				...
				for (i=-k;i!=k+1;i++)
				{
					for (j=-k;j!=k+1;j++)
					{
						code_value=code_value*2+m_reconstruct_vec[(i+black_x+q+m_new_vector_size)%m_new_vector_size][black_y][(j+black_z+p+m_new_vector_size)%m_new_vector_size];
					}
				}
				--m_exchange_code_value_black[code_value];//减去交换前由黑色交换点确定的模式
			}

			else if (abs((black_x+q+m_new_vector_size)%m_new_vector_size-white_x)>k || abs((black_z+p+m_new_vector_size)%m_new_vector_size-white_z)>k)
			{
				code_value=0;
				for (i=-k;i!=k+1;i++)
				{
			...
			for (i=-k;i!=k+1;i++)
			{
				for (j=-k;j!=k+1;j++)
				{
					code_value=code_value*2+m_reconstruct_vec[white_x][(i+white_y+q+m_new_vector_size)%m_new_vector_size][(j+white_z+p+m_new_vector_size)%m_new_vector_size];
				}
			}
			--m_exchange_code_value_white[code_value];//减去交换前由白色交换点确定的模式
			...
			if (white_x!=black_x)
			{
				...
				for (i=-k;i!=k+1;i++)
				{
					for (j=-k;j!=k+1;j++)
					{
						code_value=code_value*2+m_reconstruct_vec[black_x][(i+black_y+q+m_new_vector_size)%m_new_vector_size][(j+black_z+p+m_new_vector_size)%m_new_vector_size];
					}
				}
				--m_exchange_code_value_black[code_value];//减去交换前由黑色交换点确定的模式
			}
			else if (abs((black_y+q+m_new_vector_size)%m_new_vector_size-white_y)>k || abs((black_z+p+m_new_vector_size)%m_new_vector_size-white_z)>k)
			{
				code_value=0;
				for (i=-k;i!=k+1;i++)
				{
			...
			pair_code_value=m_exchange_code_value_white.insert(make_pair(code_value,1));//加上交换前由白色交换点确定的模式
			if(!pair_code_value.second)           
			{
				++pair_code_value.first->second; 
			}
			...
			if (white_z!=black_z)
			{
				...
				pair_code_value=m_exchange_code_value_black.insert(make_pair(code_value,1));
			...
			for (i=-k;i!=k+1;++i)
			{
				for (j=-k;j!=k+1;++j)
				{
					code_value=code_value*2+m_reconstruct_vec[white_x][(j+white_y+p+m_new_vector_size)%m_new_vector_size][(i+white_z+q+m_new_vector_size)%m_new_vector_size];
				}
			}
			pair_code_value=m_exchange_code_value_white.insert(make_pair(code_value,1));//加上交换前由白色交换点确定的模式
			if(!pair_code_value.second)           
			...
			if (white_x!=black_x)
			{
				...
				for (i=-k;i!=k+1;i++)
				{
					for (j=-k;j!=k+1;j++)
					{
						code_value=code_value*2+m_reconstruct_vec[black_x][(j+black_y+p+m_new_vector_size)%m_new_vector_size][(i+black_z+q+m_new_vector_size)%m_new_vector_size];
					}
				}
				pair_code_value=m_exchange_code_value_black.insert(make_pair(code_value,1));
				if(!pair_code_value.second)           //加上交换前由黑色交换点确定的模式
	...
	for(iter_Success;iter_Success!=m_exchange_code_value_white.end();++iter_Success)
	{
		//190113
		Engerry_before_exchange_white=Engerry_before_exchange_white+abs(m_reconstruct_code_value[iter_Success->first]-3*m_new_vector_size*m_train_code_value[iter_Success->first]*(m_multiple*m_multiple));//计算没有变化的模式个数
		m_reconstruct_code_value[iter_Success->first]=m_reconstruct_code_value[iter_Success->first]+m_exchange_code_value_white[iter_Success->first];//保存原来的模式
		Engerry_after_exchange_white=Engerry_after_exchange_white+abs(m_reconstruct_code_value[iter_Success->first]-3*m_new_vector_size*m_train_code_value[iter_Success->first]*(m_multiple*m_multiple));//计算模式差值
	}
	...
	for(iter_Success;iter_Success!=m_exchange_code_value_black.end();++iter_Success)
	{
		Engerry_before_exchange_black=Engerry_before_exchange_black+abs(m_reconstruct_code_value[iter_Success->first]-3*m_new_vector_size*m_train_code_value[iter_Success->first]*(m_multiple*m_multiple));//计算没有变化的模式个数
		m_reconstruct_code_value[iter_Success->first]=m_reconstruct_code_value[iter_Success->first]+m_exchange_code_value_black[iter_Success->first];//保存原来的模式
		Engerry_after_exchange_black=Engerry_after_exchange_black+abs(m_reconstruct_code_value[iter_Success->first]-3*m_new_vector_size*m_train_code_value[iter_Success->first]*(m_multiple*m_multiple));//计算模式差值
	}
	...
	Engerry_down=m_Enerry_origial+Engerry_change_white+Engerry_change_black;  //传递交换后的能量差
...
{  
	...
	{
		m_reconstruct_code_value[iter_F->first]=m_reconstruct_code_value[iter_F->first]-m_exchange_code_value_black[iter_F->first];
	}
	iter_F = m_exchange_code_value_white.begin();
	for(iter_F;iter_F!=m_exchange_code_value_white.end();++iter_F)
	{
		m_reconstruct_code_value[iter_F->first]=m_reconstruct_code_value[iter_F->first]-m_exchange_code_value_white[iter_F->first];
	}
...
{
	m_reconstruct_vec[m_white_posx[m_white_randsite]][m_white_posy[m_white_randsite]][m_white_posz[m_white_randsite]]=1;
	m_reconstruct_vec[m_black_posx[m_black_randsite]][m_black_posy[m_black_randsite]][m_black_posz[m_black_randsite]]=0;
}
	...
	for (i=0;i!=m_vector_size;++i)
	{
		for (j=0;j!=m_vector_size;++j)
		{
			...
			for (p=-k;p!=k+1;p++)
			{
				for (q=-k;q!=k+1;q++)
				{
					if (i+p>=0 && i+p<m_vector_size && j+q>=0 && j+q<m_vector_size)
					{
...
{ 
	...
	for (q=-k;q!=k+1;q++)      
	{
		for (p=-k;p!=k+1;p++)
		{
			...
			m_reconstruct_vec[black_x][black_y][black_z]=0;
			////////////////////////////////////////////////////XY平面交换点决定的模式
			if ((white_x+q>=k) && (white_x+q<m_vector_size-k) && (white_y+p>=k) && (white_y+p<m_vector_size-k))
			{
				code_value=0;
				for (i=-k;i!=k+1;i++)
				{
					for (j=-k;j!=k+1;j++)
					{
						code_value=code_value*2+m_reconstruct_vec[i+white_x+q][j+white_y+p][white_z];
					}
				}
				--m_exchange_code_value_white[code_value];//减去交换前由白色交换点确定的模式
			}
			if ((black_x+q>=k) && (q+black_x<m_vector_size-k) && (black_y+p>=k) && (black_y+p<m_vector_size-k))
			{
				if (white_z!=black_z)
				{
					code_value=0;
					for (i=-k;i!=k+1;i++)
					{
						for (j=-k;j!=k+1;j++)
						{
							code_value=code_value*2+m_reconstruct_vec[i+black_x+q][j+black_y+p][black_z];
						}
					}
					--m_exchange_code_value_black[code_value];//减去交换前由黑色交换点确定的模式
				}
			...
			////////////////////////////////////////////////////XZ平面交换点决定的模式
			if ((white_x+q>=k) && (white_x+q<m_vector_size-k) && (white_z+p>=k) && (white_z+p<m_vector_size-k))
			{
				code_value=0;
				for (i=-k;i!=k+1;i++)
				{
					for (j=-k;j!=k+1;j++)
					{
						code_value=code_value*2+m_reconstruct_vec[i+white_x+q][white_y][j+white_z+p];
					}
				}
				--m_exchange_code_value_white[code_value];//减去交换前由白色交换点确定的模式
			}
			if ((black_x+q>=k) && (q+black_x<m_vector_size-k) && (black_z+p>=k) &&(black_z+p<m_vector_size-k))
			{
				if (white_y!=black_y)
				{
					code_value=0;
					for (i=-k;i!=k+1;i++)
					{
						for (j=-k;j!=k+1;j++)
						{
							code_value=code_value*2+m_reconstruct_vec[i+black_x+q][black_y][j+black_z+p];
						}
					}
					--m_exchange_code_value_black[code_value];//减去交换前由黑色交换点确定的模式
				}
			...
			//////////////////////////////////////////////////YZ平面交换点决定的模式
			if ((white_y+q>=k) && (white_y+q<m_vector_size-k) && (white_z+p>=k) && (white_z+p<m_vector_size-k))
			{
				code_value=0;
				for (i=-k;i!=k+1;i++)
				{
					for (j=-k;j!=k+1;j++)
					{
						code_value=code_value*2+m_reconstruct_vec[white_x][i+white_y+q][j+white_z+p];
					}
				}
				--m_exchange_code_value_white[code_value];//减去交换前由白色交换点确定的模式
			}
			if ((black_y+q>=k) && (q+black_y<m_vector_size-k) && (black_z+p>=k) &&(black_z+p<m_vector_size-k))
			{
				if (white_x!=black_x)
				{
					code_value=0;
					for (i=-k;i!=k+1;i++)
					{
						for (j=-k;j!=k+1;j++)
						{
							code_value=code_value*2+m_reconstruct_vec[black_x][i+black_y+q][j+black_z+p];
						}
					}
					--m_exchange_code_value_black[code_value];//减去交换前由黑色交换点确定的模式
				}
			...
			////////////////////////////////////////////////////YZ平面交换点决定的模式
			if ((white_z+q>=k) && (white_z+q<m_vector_size-k) && (white_y+p>=k) && (white_y+p<m_vector_size-k))
			{
				code_value=0;
				for (i=-k;i!=k+1;++i)
				{
					for (j=-k;j!=k+1;++j)
					{
						code_value=code_value*2+m_reconstruct_vec[white_x][j+white_y+p][i+white_z+q];
					}
				}
				pair_code_value=m_exchange_code_value_white.insert(make_pair(code_value,1));//加上交换前由白色交换点确定的模式
				if(!pair_code_value.second)           
			...
			if ((black_z+q>=k) && (q+black_z<m_vector_size-k) && (black_y+p>=k) &&(black_y+p<m_vector_size-k))
			{
				if (white_x!=black_x)
				{
					code_value=0;
					for (i=-k;i!=k+1;i++)
					{
						for (j=-k;j!=k+1;j++)
						{
							code_value=code_value*2+m_reconstruct_vec[black_x][j+black_y+p][i+black_z+q];
						}
					}
					pair_code_value=m_exchange_code_value_black.insert(make_pair(code_value,1));
					if(!pair_code_value.second)           //加上交换前由黑色交换点确定的模式
	...
	for(iter_Success;iter_Success!=m_exchange_code_value_white.end();++iter_Success)
	{
		Engerry_before_exchange_white=Engerry_before_exchange_white+abs(m_reconstruct_code_value[iter_Success->first]-3*m_vector_size*m_train_code_value[iter_Success->first]);//计算没有变化的模式个数
		m_reconstruct_code_value[iter_Success->first]=m_reconstruct_code_value[iter_Success->first]+m_exchange_code_value_white[iter_Success->first];//保存原来的模式
		Engerry_after_exchange_white=Engerry_after_exchange_white+abs(m_reconstruct_code_value[iter_Success->first]-3*m_vector_size*m_train_code_value[iter_Success->first]);//计算模式差值
	}
	Engerry_change_white=Engerry_after_exchange_white-Engerry_before_exchange_white;
	...
	for(iter_Success;iter_Success!=m_exchange_code_value_black.end();++iter_Success)
	{
		Engerry_before_exchange_black=Engerry_before_exchange_black+abs(m_reconstruct_code_value[iter_Success->first]-3*m_vector_size*m_train_code_value[iter_Success->first]);//计算没有变化的模式个数
		m_reconstruct_code_value[iter_Success->first]=m_reconstruct_code_value[iter_Success->first]+m_exchange_code_value_black[iter_Success->first];//保存原来的模式
		Engerry_after_exchange_black=Engerry_after_exchange_black+abs(m_reconstruct_code_value[iter_Success->first]-3*m_vector_size*m_train_code_value[iter_Success->first]);//计算模式差值
	}
	Engerry_change_black=Engerry_after_exchange_black-Engerry_before_exchange_black;
					...
					if (m_reconstruct_vec[i][j][k]==1)
					{
						for (q=-1;q!=2;q++)
						{
							for (p=-1;p!=2;p++)
							{
								for (g=-1;g!=2;g++)
								{
									if (m_reconstruct_vec[(i+q+m_vector_size)%m_vector_size][(j+p+m_vector_size)%m_vector_size][(k+g+m_vector_size)%m_vector_size]==0)
									{
					...
					else
					{
						for (q=-1;q!=2;q++)
						{
							for (p=-1;p!=2;p++)
							{
								for (g=-1;g!=2;g++)
								{
									if (m_reconstruct_vec[(i+q+m_vector_size)%m_vector_size][(j+p+m_vector_size)%m_vector_size][(k+g+m_vector_size)%m_vector_size]==1)
									{
...
void CAnnealing::Set_inital_to_reconstruct_uncondition_3()
{
	...
	for (i=0;i!=m_vector_size;++i)
	{
		...
		{  
			...
			for (k=0;k!=m_vector_size;k++)
			{
				if (i%2==0 && j%2==0 && k%2==0)
				{
					m_reconstruct_vec[i][j][k]=m_initial_rand_data[i/2][j/2][k/2];     /////将上层重建的结果传递到对应网格点
	...
	for (i=0;i!=m_vector_size;++i)
	{
		for (j=0;j!=m_vector_size;++j)
		{
			if (m_initial_imgvec[i*distance_point][j*distance_point]==1)/////统计对应网格白色点数
			{
				inital_white_number++;
			}
		}
	...
	while (add_white_number!=0)
	{
		...
		if ((point_y%2==1 || point_x%2==1 || point_z%2==1) && m_reconstruct_vec[point_x][point_y][point_z]==2)
		{
			m_reconstruct_vec[point_x][point_y][point_z]=1;
			add_white_number--;
		}
	...
	for (i=0;i!=m_vector_size;i++)
	{
		for (j=0;j!=m_vector_size;j++)
		{
			for (k=0;k!=m_vector_size;k++)
			{
				if ((i%2==1 || j%2==1 || k%2==1) && m_reconstruct_vec[i][j][k]==2)
				{
					m_reconstruct_vec[i][j][k]=0;
				}
			}
	...
	for (i=0;i!=m_vector_size;i++)
	{
		for (j=0;j!=m_vector_size;j++)
		{
			for (k=0;k!=m_vector_size;k++)
			{
				...
				/////////////////////////////////////////////////////////////////////
				if (i%2==1 && j%2==1 && k%2==1 && m_preproc_vector[i][j][k]==2)
				{
					sum_xy=0;
					sum_xz=0;
					sum_yz=0;
					for (q=-1;q!=2;q++)
					{
						for (p=-1;p!=2;p++)
						{
							sum_xy=sum_xy+m_reconstruct_vec[(i+q+m_vector_size)%m_vector_size][(j+p+m_vector_size)%m_vector_size][k];
							sum_xz=sum_xz+m_reconstruct_vec[(i+q+m_vector_size)%m_vector_size][j][(k+p+m_vector_size)%m_vector_size];
							sum_yz=sum_yz+m_reconstruct_vec[i][(j+p+m_vector_size)%m_vector_size][(k+p+m_vector_size)%m_vector_size];
						}
					}
					if (m_reconstruct_vec[i][j][k]==1 && sum_xy!=9 && sum_xz!=9 && sum_yz!=9)
					{
				...
				/////////////////////////////////////////////////////////////////////
				else if (i%2==1 && j%2==1 && k%2==0 && m_preproc_vector[i][j][k]==2)
				{
					sum_3D=0;
					sum_xy=0;
					sum_xz=0;
					...
					for (q=-1;q!=2;q++)
					{
						for (p=-1;p!=2;p++)
						{
							sum_xy=sum_xy+m_reconstruct_vec[(i+q+m_vector_size)%m_vector_size][(j+p+m_vector_size)%m_vector_size][k];
							sum_xz=sum_xz+m_reconstruct_vec[(i+q+m_vector_size)%m_vector_size][j][(k+p+m_vector_size)%m_vector_size];
							sum_yz=sum_yz+m_reconstruct_vec[i][(j+q+m_vector_size)%m_vector_size][(k+p+m_vector_size)%m_vector_size];
							for (g=-1;g!=2;g++)
							{
								sum_3D=sum_3D+m_reconstruct_vec[(i+q+m_vector_size)%m_vector_size][(j+p+m_vector_size)%m_vector_size][(k+g+m_vector_size)%m_vector_size];
							}
						}
					}
					if (sum_3D==27)
				...
				/////////////////////////////////////////////////////////////////////
				else if (i%2==1 && j%2==0 && k%2==1 && m_preproc_vector[i][j][k]==2)
				{
					sum_3D=0;
					sum_xy=0;
					sum_xz=0;
				...
				/////////////////////////////////////////////////////////////////////
				else if (i%2==0 && j%2==1 && k%2==1 && m_preproc_vector[i][j][k]==2)
				{
					sum_3D=0;
					sum_xy=0;
					sum_xz=0;
				...
				/////////////////////////////////////////////////////////////////////
				else if (i%2==0 && j%2==0 && k%2==1 && m_preproc_vector[i][j][k]==2)
				{
					sum_3D=0;
					sum_xy=0;
					sum_xz=0;
				...
				/////////////////////////////////////////////////////////////////////
				else if (i%2==0 && j%2==1 && k%2==0 && m_preproc_vector[i][j][k]==2)
				{
					sum_3D=0;
					sum_xy=0;
					sum_xz=0;
				...
				/////////////////////////////////////////////////////////////////////
				else if (i%2==1 && j%2==0 && k%2==0 && m_preproc_vector[i][j][k]==2)
				{
					sum_3D=0;
					sum_xy=0;
					sum_xz=0;
	...
	for (i=0;i!=m_vector_size;i++)
	{
		for (j=0;j!=m_vector_size;j++)
		{
			for (k=0;k!=m_vector_size;k++)
			{
				...
				if ((sum_xy==8 || sum_xz==8 || sum_yz==8 ) && m_reconstruct_vec[i][j][k]==0 && m_preproc_vector[i][j][k]==2)
				{
					temporay_vector[i][j][k]=0;
				}
			}
